...
**Not yet implemented (Phase 4):** multi-user isolation, session sharing / read-only viewers, mobile phase-2 extras (PWA, pinch-zoom), a mobile settings entry point, multi-server "Connected Servers" (design agreed in `docs/MULTI-SERVER-PLAN.md`: SSH + remote tmux, server registry, sidebar grouping).
