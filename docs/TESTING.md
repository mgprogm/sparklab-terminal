...
| `apps/terminal/.../auth/login-screen.test.tsx`                 | Login form rendering, username/password submission, invalid-credentials error display, and stealth reveal (desktop Ctrl+Space toggle; mobile 3s press-and-hold reveals the form, early release keeps it hidden)                                                                                               | 5     |
| `apps/terminal/.../auth/use-press-and-hold.test.ts`            | Press-and-hold gesture hook (mobile stealth reveal): fires only after a full uninterrupted hold; cancels on release, pointer-drift beyond tolerance, and pointercancel; jitter within tolerance keeps holding; ignores non-primary mouse button; no-ops when disabled; suppresses the long-press context menu | 8     |
