...
| `SCHEDULED_TERMINAL_ACTIONS_KEY` | _(unset)_                                     | For delayed text       | Stable base64 32-byte key for encrypted Agent Chat delayed text + keys. Generate once with `openssl rand -base64 32`; missing key leaves named-key timers available but rejects delayed text. |
...
| `GPT56TERRA_DEPLOYMENT`                                     | _(unset)_               | No                  | Optional Azure AI Foundry deployment that enables `gpt-5.6-terra`.                                                        |
| `GPT56LUNA_DEPLOYMENT`                                      | _(unset)_               | No                  | Optional Azure AI Foundry deployment that enables `gpt-5.6-luna`.                                                         |
