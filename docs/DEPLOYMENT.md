...
| `HOST`                    | `127.0.0.1`                                   | No                     | Gateway bind address. **Must be loopback unless `GATEWAY_AUTH_TOKEN` is set.**                                                                       |
| `GATEWAY_AUTH_TOKEN`      | _(unset)_                                     | **Yes**                | Shared secret for single-user auth. Unset + non-loopback HOST → gateway refuses to start.                                                            |
...
```bash
openssl rand -hex 32
# → e.g. a3f9b2c1d0e4f5a6b7c8d9e0f1a2b3c4d5e6f7a8b9c0d1e2f3a4b5c6d7e8f9a0
```
...
| Threat                                 | Mitigation                                                                                                          |
| -------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| Unauthenticated shell access           | `GATEWAY_AUTH_TOKEN` required; all `/api/*` except `/api/auth/*`, plus `/attach`, check the `gw_session` cookie.    |
...
| Non-loopback bind without token        | Gateway refuses to start (`process.exit(1)`) if `GATEWAY_AUTH_TOKEN` is unset and `HOST` is non-loopback.           |

## Development (open mode)

When `GATEWAY_AUTH_TOKEN` is unset and `HOST` is a loopback address (`127.0.0.1`, `::1`, `localhost`), the gateway runs in **open mode**: auth and origin checks are disabled, matching pre-Phase-3 behavior. This lets test scripts and `curl` work without tokens or browser headers.
...
Phase 3 changes the default `HOST` bind from the implicit `0.0.0.0` to explicit `127.0.0.1`. **If you were relying on LAN access directly to the gateway, traffic now goes through the reverse proxy.** Update your `ALLOWED_ORIGINS` and `GATEWAY_AUTH_TOKEN` accordingly.
