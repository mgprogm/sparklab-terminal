...
- **tmux is the source of truth for live state.** There is no database of
  running processes. A restarted gateway rediscovers live sessions via
  `tmux has-session` / `tmux ls`. Display metadata (name, org/project, mute
  state) lives in `data/sessions.json` via `src/metadata.js` and deliberately
  outlives tmux: when a reachable server no longer reports a tracked session,
  the gateway returns a persistent `alive:false` placeholder until the user
  deletes it.
- **Unreachable is not ended.** A server outage reconstructs last-known rows
  with `reachable:false`; a missing tmux session on a reachable server produces
  `reachable:true, alive:false`. Neither state is auto-pruned. This preserves
  user context without claiming that an unreachable remote process is dead.
...
The proxy validates DNS once and connects to that exact checked address, so
public literal-IP URLs work without weakening private/reserved-network blocks.
Model-facing browser state is compacted independently from the ephemeral
screenshot, and aggregate performance data is available from `/health` without
request labels or sensitive bodies.
...
- `src/server.js` — REST session CRUD (`/api/sessions`), a read-only scrollback endpoint (`GET /api/sessions/:id/scrollback`, tmux `capture-pane`), the `/attach` WebSocket endpoint, tmux session management. The list response carries per-session status (`attachedClients`, `lastActivity`, `reachable`, `alive`) and synthesizes last-known unreachable and ended-session rows from metadata. Spawns ptys with `encoding: null` so all terminal I/O stays raw bytes.
- `src/metadata.js` — session display metadata (name, org/project, mute state) persisted to `data/sessions.json` (gitignored runtime data). `GATEWAY_DATA_DIR` can redirect this sidecar for isolated tests.
...
| `src/tools.ts`                                                             | OpenAI function schemas for terminal reads/writes and isolated virtual-browser tools, including one-time-approved `browser_capture`. No `kill_session`, raw MCP, JavaScript, or CDP capability.                                                                                                                                     |
| `src/gateway-client.ts`                                                    | Fetch client for the gateway (loopback); logs in with `GATEWAY_AUTH_*` when auth is on, reuses the `gw_session` cookie, and carries bounded screenshot bytes to the selected session's `fs/upload` route.                                                                                                                           |
...
| `src/browser-runtime.ts` / `src/browser-state.ts` / `src/browser-proxy.ts` | Lazy Browser Use MCP lifecycle, bounded model-facing state, public literal-IP support, single-resolution address pinning, and public-network-only egress enforcement.                                                                                                                                                               |
| `src/browser-performance-metrics.ts`                                       | Process-wide numeric launch/readiness/call/payload/frame aggregates exposed by `/health`; accepts no URLs, text, coordinates, tokens, identifiers, or image bodies.                                                                                                                                                                 |
...
| `components/session-sidebar.tsx` / `components/session-list.tsx` | Sidebar composition plus the grouped server → org → project session tree, create/reconnect dialogs, ended/unreachable status affordances, session action menu, delete confirmation, and settings entry point.                                                                                                                                                                                                                                                                                                    |
...
The approval card presents the exact terminal and absolute destination for
`browser_capture`; the image file is written only by the gateway on that
terminal's server and the screenshot/result remain absent from durable chat
history.
