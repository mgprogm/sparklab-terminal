...
| `components/settings-dialog.tsx` | Settings modal (shadcn Dialog) with four sections: Appearance (terminal font size — the only one with behavior), Agent chat (read-only model + per-write approval policy), Account (username + sign out; password is env-set), Connection (gateway URL, status, session count). All client-only — no gateway changes.                                                                                                                                                                                            |
...
| `hooks/use-url-flag-sync.ts`     | Generic presence-flag version of the session sync for booleans: `?settings` ↔ settings dialog open, `?agent` ↔ agent panel open. Presence opens on mount (deferring to the current/persisted value when absent — never force-closes); reflects the flag back to the URL. Instances compose (each write preserves the other params).                                                                                                                                                                              |
...
| `store.ts`                       | Zustand (+`persist`): persists `activeSessionId`, `sidebarCollapsed`, and `terminalFontSize`; holds ephemeral `mobileSidebarOpen` / `settingsOpen`; fallback when the active session vanishes.                                                                                                                                                                                                                                                                                                                   |
