...
| `SCHEDULED_TERMINAL_ACTIONS_KEY` | _(unset)_                                     | gateway         | Base64 32-byte key required to schedule delayed text + keys; generate with `openssl rand -base64 32`. Keep it stable so pending actions survive restart.                             |
