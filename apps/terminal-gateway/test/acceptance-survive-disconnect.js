...
// Multi-server: the REST id is now QUALIFIED (`<serverId>/web-<uuid>`). Direct
// `tmux` CLI calls need the bare tmux name (the last "/" segment).
...
function cleanup() {
  if (SESSION) {
    try {
      execFileSync("tmux", ["kill-session", "-t", bare(SESSION)], {
        stdio: "ignore",
      });
    } catch {}
  }
...
async function main() {
  ...
  if (res.status !== 201)
    fail(`POST /api/sessions returned ${res.status}, expected 201`);
  SESSION = (await res.json()).id;
  URL = `ws://localhost:${PORT}/attach?session=${encodeURIComponent(SESSION)}`;
  ...
  console.log(
    `connection #1: ticks streaming, last tick before disconnect = ${before}`,
  );
  ...
  if (!aliveMidGap)
    fail("tmux session died during the disconnect gap (job did NOT survive)");
  ...
  if (delta < 8)
    fail(
      `tick advanced only ${delta}; expected ~${elapsedSec} (loop did not keep running)`,
    );
  if (Math.abs(delta - elapsedSec) > 5) {
    fail(
      `delta ${delta} not within 5 of real gap ${elapsedSec}s — timing does not match a live-running loop`,
    );
  }
  // (b) ticks are still arriving live after reconnect.
  if (!(afterSecond > afterFirst))
    fail(
      `ticks not advancing live after reconnect (${afterFirst} -> ${afterSecond})`,
    );
  ...
  console.log(
    `  before=${before} -> after=${afterFirst} (+${delta} while disconnected ~${elapsedSec}s) -> live=${afterSecond}`,
  );
  cleanup();
  process.exit(0);
}
