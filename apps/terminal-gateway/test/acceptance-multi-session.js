...
function listWebSessions() {
  try {
    ...
    return out.split('\n').map((s) => s.trim()).filter((s) => s.startsWith('web-'));
  } catch {
    return [];
  }
}
...
function cleanup() {
  ...
  for (const id of createdIds) {
    try { execFileSync('tmux', ['kill-session', '-t', id], { stdio: 'ignore' }); } catch {}
  }
...
function closeWs(state) {
  return new Promise((resolve) => {
    ...
    try { state.ws.close(); } catch { resolve(); }
...
async function main() {
  ...
  if (resA.status !== 201 || resB.status !== 201) {
    fail(`POST /api/sessions returned ${resA.status} / ${resB.status}, expected 201`);
  }
  const A = (await resA.json()).id;
  const B = (await resB.json()).id;
  createdIds.push(A, B);
  console.log(`created A=${A}  B=${B}`);
  if (!tmuxHasSession(A) || !tmuxHasSession(B)) fail('created sessions not present in tmux');

  // --- 2. Attach + start distinct jobs, then check isolation ---
  const wsA = await openWs(A);
  const wsB = await openWs(B);
  await sleep(600);
  wsA.ws.send(Buffer.from('while true; do echo AAA $((i=i+1)); sleep 1; done\n'), { binary: true });
  wsB.ws.send(Buffer.from('while true; do echo BBB $((j=j+1)); sleep 1; done\n'), { binary: true });
  ...
  if (wsA.buf.includes('BBB')) fail("session A's stream leaked BBB output (no isolation)");
  if (wsB.buf.includes('AAA')) fail("session B's stream leaked AAA output (no isolation)");
  console.log(`isolation OK: A sees only AAA (last=${aTick}), B sees only BBB (last=${bTick})`);
  ...
  await sleep(4000);
  if (!tmuxHasSession(A)) fail('session A died after we disconnected from it (job did NOT survive tab switch)');
  if (!tmuxHasSession(B)) fail('session B died while active');
  const bMid = maxCounter(wsB.buf, 'BBB');
  console.log(`during switch: A alive=${tmuxHasSession(A)}, B alive=${tmuxHasSession(B)}, B counting (BBB=${bMid})`);
  ...
  if (!(aAfter > aBefore)) {
    fail(`A's counter did not advance while we were away (before=${aBefore}, after=${aAfter}) — job stalled on detach`);
  }
  console.log(`switched back to A: counter advanced ${aBefore} -> ${aAfter} while unwatched (job kept running)`);

  // --- 4. DELETE A (the ONE surgical kill). B must be untouched. ---
  const bBeforeDelete = maxCounter(wsB.buf, 'BBB');
  const delRes = await rest('DELETE', `/api/sessions/${A}`);
  if (delRes.status !== 204) fail(`DELETE A returned ${delRes.status}, expected 204`);
  await closeWs(wsA2);
  await sleep(500);
  if (tmuxHasSession(A)) fail('DELETE did not kill session A');
  if (!tmuxHasSession(B)) fail('DELETE A also killed B — delete was NOT surgical!');
  // B's job must still be counting.
  await sleep(3000);
  const bAfterDelete = maxCounter(wsB.buf, 'BBB');
  if (!(bAfterDelete > bBeforeDelete)) {
    fail(`B's job stopped counting after A was deleted (${bBeforeDelete} -> ${bAfterDelete})`);
  }
  console.log(`DELETE A surgical: has-session A=${tmuxHasSession(A)} (killed), B=${tmuxHasSession(B)} (alive), B still counting ${bBeforeDelete} -> ${bAfterDelete}`);

  // --- 5. Clean up B, verify no orphans ---
  await closeWs(wsB);
  const delResB = await rest('DELETE', `/api/sessions/${B}`);
  if (delResB.status !== 204) fail(`DELETE B returned ${delResB.status}, expected 204`);
  await sleep(300);
  const orphans = listWebSessions().filter((s) => createdIds.includes(s));
  if (orphans.length) fail(`orphan web- sessions from this test remain: ${orphans.join(', ')}`);
  console.log(`cleanup OK: no orphan web- sessions remain (${A}, ${B} both gone)`);

  console.log('\nPASS: multi-session isolation, jobs survive tab switching, DELETE is the only surgical kill.');
  console.log(`  isolation: A=AAA-only(last ${aTick}), B=BBB-only(last ${bTick})`);
  console.log(`  survived switch: A advanced ${aBefore} -> ${aAfter} while unwatched`);
  console.log(`  surgical delete: A killed, B survived and kept counting ${bBeforeDelete} -> ${bAfterDelete}`);
  cleanup();
  process.exit(0);
}
